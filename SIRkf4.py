...
# SIRkf3.py, here the assimilation step uses all measured data up to
# the current data point. Ensemble measurements are adjusted
# accordingly. This most closely resembles Evensen's Ensemble
# Smoother.

# Use:
#    SIRkf4.py <DataFileName> <DataNoise> <Tspan>
...
for i in range(Ndata_pts):
    ...
    DatCov = np.diag(math.pow(DataNoise,2)*np.ones((i+1)),0)

    # Pull out data array and add random perturbations.
    Data = np.tile(DataArray[:(i+1),2],(EnSize,1)).transpose()\
           + DataNoise*rn.randn((i+1),EnSize)
    ...
    for j in range(i+1):
        [EnsArray, EnsTime] = SIR_ensemble_lib.SIRens(ParamArray,Tinit,DataArray[j,0],
                                                      (j+1)*Ntimestep)
        ...
        if (j == 0):
            ...
            EnsObs = np.vstack([EnsObs,SIR_ensemble_lib.SIRmeasure(EnsArray)])
