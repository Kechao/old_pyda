...
# SIR data.  It uses no re-propagation through SIR to maintain SIR
# balances. After enKF analysis it simply ensures (S,I) are between 0
# and 1.0. Analysis forecasts are then done starting at the most
# recent data point.

# Use:
#    SIRkf2.py <DataFileName> <DataNoise> <Tspan>
...
for i in range(Ndata_pts):
    ...
    # Since enKF assumes Gaussianity we must adjust to maintain
    # positivity and to not exceed 1.0. This still could allow
    # starting S0+I0 > 1.0.
    AnalysisParams = np.maximum(AnalysisParams,0.0)
    AnalysisEnsemble = np.maximum(AnalysisEnsemble,0.0)
    AnalysisEnsemble = np.minimum(AnalysisEnsemble,1.0)
