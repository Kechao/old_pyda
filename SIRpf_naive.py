...
# SIR data using the most naive version of particle filtering
# (PF). Weighting is done using the 'forward likelihood' as an
# importance sampler and resampling is done just using the
# weights. The ensemble is generated deterministically so the
# assimilation is highly degenerate. No repropogation through the
# model takes place. Propagation is only from current data
# point. However, due to the nature of the filter-resample scheme
# balances are maintained.

# Use:
#    SIRpf_naive.py <DataFileName> <DataNoise> <Tspan>
...
for i in range(Ndata_pts):
    ...
    [AnalysisCast,AnCastTime] = SIR_ensemble_lib.SIRens(AnalysisParams,Tstop,Tspan,
                                                        Ncaststep-i*Ntimestep)
